...
//  Serveur Express avec OAuth2 Discord + Supabase
// ============================================================
...
// Config : variables d'environnement (Railway) ou fichier local (développement)
let config;
try {
  config = require('./config/discord.config');
} catch (e) {
  // En production sur Railway, on utilise les variables d'environnement
  config = {
...
// ── CLIENT SUPABASE ─────────────────────────────────────────
// On utilise la clé service_role côté serveur uniquement
...
// Convertit les données DB au format attendu par le frontend
...
app.get('/auth/discord/callback', async (req, res) => {
  ...
  try {
    ...
    if (isAdmin) {
      ...
      console.log(`✅ Admin connecté : ${userData.username}`);
...
app.get('/api/admin/check', (req, res) => {
  ...
  const isAdmin = !!req.session.isAdmin ||
    (user && config.ADMIN_ROLE_IDS.some(id => user.roles.includes(id)));
  res.json({ isAdmin });
});
...
app.post('/api/admin/save', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (d.categories?.length) {
      ...
      for (const cat of d.categories) {
        await supabase.from('categories').upsert({
          ...
          color: cat.color || '#ffffff', icon: cat.icon || '📦',
    ...
    if (d.mods) {
      ...
      if (ids.length > 0) {
        await supabase.from('mods').delete().not('id', 'in', `(${ids.map(i => `"${i}"`).join(',')})`);
      } else {
        await supabase.from('mods').delete().neq('id', '');
      }
      for (const mod of d.mods) {
        console.log(`💾 Sauvegarde mod "${mod.name}" — images:`, JSON.stringify(mod.images));
        await supabase.from('mods').upsert({
          id: mod.id, name: mod.name, category: mod.category,
          description: mod.description || '', image: mod.image || '',
          base_price: mod.basePrice || 0, images: mod.images || [], featured: mod.featured || false,
          visible: mod.visible !== false, position: mod.position ?? 0
        }, { onConflict: 'id' });
      }
    }
    ...
    if (d.promotions) {
      ...
      if (ids.length > 0) {
        await supabase.from('promotions').delete().not('id', 'in', `(${ids.map(i => `"${i}"`).join(',')})`);
      } else {
    ...
    if (d.discordRoles) {
      ...
      if (ids.length > 0) {
        await supabase.from('discord_roles').delete().not('role_id', 'in', `(${ids.map(i => `"${i}"`).join(',')})`);
      } else {
        await supabase.from('discord_roles').delete().neq('role_id', '');
      }
      for (const role of d.discordRoles) {
        await supabase.from('discord_roles').upsert({
          role_id: role.roleId, role_name: role.roleName,
          discount: role.discount || 0, color: role.color || '#ffffff'
        }, { onConflict: 'role_id' });
      }
    }
