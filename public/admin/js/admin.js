...
async function loadOrders() {
  ...
  try {
    ...
    const ordersHTML = orders.map(order => {
      ...
      const itemsList = items.map(i => `<span style="font-size:0.78rem;color:var(--grey-l)">${esc(i.name)}</span>`).join(', ');

      return `
        <div class="item-row order-row" data-status="${status}">
          <div style="width:10px;height:10px;border-radius:50%;background:${color};flex-shrink:0"></div>
          <div class="item-info">
            <div class="item-name">${esc(order.discord_username || 'Visiteur')}</div>
            <div class="item-meta">${date} · ${items.length} article${items.length > 1 ? 's' : ''} · #${esc(order.ticket_channel || '')}</div>
            <div style="margin-top:4px">${itemsList}</div>
          </div>
          <span class="item-badge" style="color:${color};border-color:${color}40;background:${color}12;font-family:var(--mono);font-size:0.7rem">${label}</span>
          <span style="font-family:var(--mono);font-size:0.9rem;font-weight:600;color:var(--white);flex-shrink:0">${formatEUR(order.total_price)}</span>
          <select class="order-status-select" data-id="${order.id}" onchange="changeOrderStatus('${order.id}', this)" style="background:var(--bg-2);border:1px solid var(--border-b);color:var(--white);padding:6px 10px;border-radius:var(--r);font-family:var(--font);font-size:0.8rem;cursor:pointer;outline:none;min-width:150px">
...
async function saveMod() {
  ...
  const mod = { id: idx !== '' ? data.mods[idx]?.id : 'mod-' + Date.now(), name, category: cat, description: desc, image, images, basePrice: price, featured: isToggleOn('toggleFeatured'), visible: isToggleOn('toggleVisible'), position: idx !== '' ? (data.mods[parseInt(idx)]?.position ?? parseInt(idx)) : (data.mods?.length || 0) };
  if (idx !== '') data.mods[parseInt(idx)] = mod;
  else { if (!data.mods) data.mods = []; data.mods.push(mod); }
  await saveAll(); closeModModal(); renderModsList(); renderDashboard();
