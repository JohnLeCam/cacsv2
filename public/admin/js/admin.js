...
async function loadStats() {
  ...
  try {
    ...
    const formatEURLocal = amount =>
      new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(amount || 0);

    const topModsHTML = stats.topMods.length === 0
      ? '<p style="color:var(--grey-m);font-size:0.85rem">Aucune commande enregistrée.</p>'
      : stats.topMods.map((mod, i) => `
          <div class="item-row" style="padding:10px 16px">
            <div style="font-family:var(--mono);font-size:1.1rem;color:var(--red);width:24px;flex-shrink:0">#${i+1}</div>
            <div class="item-info">
              <div class="item-name">${esc(mod.name)}</div>
              <div class="item-meta">${mod.count} commande${mod.count > 1 ? 's' : ''} · ${formatEURLocal(mod.revenue)}</div>
            </div>
          </div>
        `).join('');

    const recentOrdersHTML = stats.recentOrders.length === 0
      ? '<p style="color:var(--grey-m);font-size:0.85rem">Aucune commande enregistrée.</p>'
      : stats.recentOrders.map(order => {
          ...
          const nbItems = Array.isArray(order.items) ? order.items.length : 0;
          return `
            <div class="item-row" style="padding:10px 16px">
              <div class="item-info">
                <div class="item-name">${esc(order.discord_username || 'Visiteur')}</div>
                <div class="item-meta">${date} · ${nbItems} article${nbItems > 1 ? 's' : ''} · #${esc(order.ticket_channel)}</div>
              </div>
              <span class="item-badge badge-visible" style="font-family:var(--mono)">${formatEURLocal(order.total_price)}</span>
            </div>
          `;
        }).join('');
    ...
    const last7HTML = stats.last7Days.map(d => {
      ...
      return `
        <div style="display:flex;flex-direction:column;align-items:center;gap:6px;flex:1">
          <div style="font-family:var(--mono);font-size:0.8rem;color:var(--white)">${d.count}</div>
          <div style="width:100%;background:var(--bg-2);border-radius:3px;height:60px;display:flex;align-items:flex-end;overflow:hidden">
            <div style="width:100%;height:${pct}%;background:var(--red);border-radius:3px 3px 0 0;transition:height 0.4s ease;min-height:${d.count>0?'4px':'0'}"></div>
          </div>
          <div style="font-size:0.68rem;color:var(--grey-m);text-align:center;font-family:var(--mono)">${label}</div>
        </div>
      `;
    }).join('');
    ...
    container.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:16px;margin-bottom:24px">
        <div class="stat-card">
          <div class="stat-value red">${stats.totalOrders}</div>
          <div class="stat-label">Commandes totales</div>
        </div>
        <div class="stat-card">
          <div class="stat-value green" style="font-size:1.6rem">${formatEURLocal(stats.totalRevenue)}</div>
      ...
      <div class="card" style="margin-bottom:24px">
        <div class="card-header">
          <span class="card-title">📅 Commandes — 7 derniers jours</span>
        </div>
        <div class="card-body">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
        <div class="card">
          <div class="card-header">
            <span class="card-title">🏆 Top 5 mods commandés</span>
          </div>
          <div class="card-body" style="padding:12px">
            <div class="items-list">${topModsHTML}</div>
          </div>
        </div>

        <div class="card">
          <div class="card-header">
            <span class="card-title">🕐 10 dernières commandes</span>
          </div>
          <div class="card-body" style="padding:12px">
            <div class="items-list">${recentOrdersHTML}</div>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="margin-top:24px;border-color:rgba(217,0,0,0.2)">
        <div class="card-header">
          <span class="card-title" style="color:var(--red)">⚠️ Zone dangereuse</span>
        </div>
        <div class="card-body" style="display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap">
          <div>
            <div style="font-size:0.9rem;font-weight:600;color:var(--white);margin-bottom:4px">Réinitialiser les statistiques</div>
            <div style="font-size:0.82rem;color:var(--grey-m)">Supprime toutes les commandes enregistrées. Action irréversible.</div>
          </div>
    ...
    container.innerHTML = `<div style="color:var(--red);font-size:0.9rem">Erreur chargement des statistiques.</div>`;
    console.error(err);
  }
}

async function resetStats() {
  if (!confirm('⚠️ Supprimer TOUTES les commandes enregistrées ? Cette action est irréversible.')) return;
  try {
    ...
    if (res.ok) {
      ...
      showToast('Erreur lors du reset.', true);
    }
  } catch (e) {
...
function duplicateMod(index) {
  ...
  const copy = {
    ...
    position: data.mods.length
  };
  data.mods.push(copy);
  saveAll();
  renderModsList();
  showToast('Mod dupliqué !');
}
...
function renderPromosList() {
  ...
  if (promos.length === 0) {
    list.innerHTML = '<p style="color:var(--grey-m);font-size:0.9rem">Aucune promotion.</p>';
    return;
  }
  ...
  list.innerHTML = promos.map((p, i) => {
    ...
    return `
      <div class="item-row">
        ...
        ${statusBadge}
        <div class="item-actions">
          <button class="btn-icon" onclick="openPromoModal(${i})">✏️</button>
          <button class="btn-icon del" onclick="deletePromo(${i})">🗑️</button>
        </div>
      </div>
    `;
  }).join('');
...
function openPromoModal(index = null) {
  ...
  wrap.innerHTML = (data.categories || []).map(c => {
    const id = c.id || c; const name = c.name || c; const icon = c.icon || ''; const color = c.color || '';
    return `<label style="display:flex;align-items:center;gap:6px;cursor:pointer;background:var(--bg-2);border:1px solid var(--border-b);padding:6px 12px;border-radius:var(--r);font-size:0.82rem;color:${color || 'var(--grey-l)'};">
...
async function savePromo() {
  ...
  const promo = {
    ...
    name, description: desc, discountPercent: discount,
    endDate:           new Date(endDate).toISOString(),
    applyToCategories: cats, active
  };

  if (idx !== '') data.promotions[parseInt(idx)] = promo;
  else { if (!data.promotions) data.promotions = []; data.promotions.push(promo); }
...
function deletePromo(index) {
  ...
  saveAll(); renderPromosList(); renderDashboard();
...
function renderRolesList() {
  ...
  list.innerHTML = roles.map((r, i) => `
    <div class="item-row">
      ...
      <div class="item-actions">
        <button class="btn-icon" onclick="openRoleModal(${i})">✏️</button>
        <button class="btn-icon del" onclick="deleteRole(${i})">🗑️</button>
      </div>
    </div>
  `).join('');
}
...
async function saveRole() {
  ...
  if (idx !== '') data.discordRoles[parseInt(idx)] = role;
  else data.discordRoles.push(role);
...
function deleteRole(index) {
  ...
  saveAll(); renderRolesList(); renderDashboard();
...
function renderCategoriesList() {
  ...
  list.innerHTML = cats.map((cat, i) => `
    <div class="item-row" draggable="true" data-index="${i}"
         ondragstart="onDragStart(event,${i})" ondragover="onDragOver(event)"
         ondragleave="onDragLeave(event)" ondrop="onCatDrop(event,${i})" ondragend="onDragEnd(event)">
      ...
      <div class="item-actions">
        <button class="btn-icon" onclick="openCatModal(${i})">✏️</button>
        <button class="btn-icon del" onclick="deleteCat(${i})">🗑️</button>
      </div>
    </div>
  `).join('');
}
...
function updateCatSwatch() {
  ...
  if (/^#[0-9a-fA-F]{3,6}$/.test(val)) document.getElementById('catColorPicker').value = val;
  updateCatPreview();
}
...
async function saveCat() {
  ...
  if (idx !== '') data.categories[parseInt(idx)] = cat;
...
function deleteCat(index) {
  ...
  else { if (!confirm(`Supprimer la catégorie "${cat.name}" ?`)) return; }
  data.categories.splice(index, 1);
  saveAll(); renderCategoriesList(); renderDashboard();
...
function updateRichToolbar() {
  ...
  document.querySelectorAll('.rich-btn').forEach(btn => {
    ...
    if (cmd && cmds.includes(cmd)) btn.classList.toggle('active', document.queryCommandState(cmd));
  });
}
...
function renderImageManager() {
  ...
  if (_imageList.length === 0) {
    container.innerHTML = '<div class="img-mgr-empty">Aucune image/vidéo — ajoute une URL ci-dessous</div>';
    return;
  }
  container.innerHTML = _imageList.map((url, i) => {
    const isYT    = /youtube\.com|youtu\.be/.test(url);
    const label   = isYT ? '▶ Vidéo YouTube' : (i === 0 ? '⭐ Photo principale' : `Photo ${i+1}`);
    const preview = isYT
      ? `<div style="width:52px;height:36px;background:#ff0000;border-radius:3px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.1rem;flex-shrink:0">▶</div>`
      : `<img src="${esc(url)}" onerror="this.style.opacity=0.3" loading="lazy" style="width:52px;height:36px;object-fit:cover;border-radius:3px;flex-shrink:0;background:var(--bg-2)">`;
    return `
      <div class="img-mgr-item" draggable="true"
           ondragstart="imgDragStart(event,${i})" ondragover="imgDragOver(event)"
           ondragleave="imgDragLeave(event)" ondrop="imgDrop(event,${i})" ondragend="imgDragEnd(event)" data-idx="${i}">
        <span class="img-mgr-handle">⠿</span>
        ${preview}
        <div class="img-mgr-info">
          <span class="img-mgr-label">${label}</span>
          <span class="img-mgr-url">${esc(url.length > 45 ? url.substring(0,45)+'…' : url)}</span>
        </div>
        <div class="img-mgr-actions">
          ${i > 0 ? `<button class="img-mgr-btn" onclick="imgSetMain(${i})" title="Définir comme principal">★</button>` : ''}
...
function imgAdd() {
  ...
  if (!url) return;
  const isYT = /youtube\.com|youtu\.be/.test(url);
  if (!isYT && !url.startsWith('http')) { alert('URL invalide — doit commencer par http'); return; }
  _imageList.push(url);
  input.value = '';
  renderImageManager();
}
function imgRemove(index)  { _imageList.splice(index, 1); renderImageManager(); }
function imgSetMain(index) { const [item] = _imageList.splice(index, 1); _imageList.unshift(item); renderImageManager(); }
